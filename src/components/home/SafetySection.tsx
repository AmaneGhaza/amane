      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
            <div className={`text-center mb-12 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
              <h2 className="text-3xl md:text-4xl font-bold font-headline text-[#009736]">
              {dict.safety.title}
              </h2>
            </div>
            <Card className={`bg-card transition-all duration-700 border-2 border-[#009736]/30 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} style={{ transitionDelay: '200ms' }}>
                <CardContent className="p-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
                        {safetyFeatures.map((feature, index) => (
                            ...
                            >
                                <div className="text-[#009736]">{feature.icon}</div>
                    ...
                    <div className={`mt-8 pt-6 border-t flex items-center justify-center transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                        <div className="flex items-center space-x-2 rtl:space-x-reverse">
                            <Check className="h-5 w-5 text-[#009736]" />
